...
def index(request,pIndex=1):
    ...
    #获取并判断菜品类别
    pid = request.GET.get("category_id",None)
    if pid:
        ulist = ulist.filter(category_id=pid)
        mywhere.append('category_id='+pid)
...
def insert(request):
    ...
    try:
        ...
        if not myfile:
            return HttpResponse("没有上传封面文件信息")
