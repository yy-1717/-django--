...
urlpatterns = [
    ...
    #后台管理员登录，退出路由
     ...
     path('dologin', index.dologin, name="myadmin_dologin"),#登录
     path('logout', index.logout, name="myadmin_logout"),#退出
     path('verify', index.verify, name="myadmin_verify"),#退出
    ...
    #菜品类别信息管理路由
     ...
     path('category/load/<int:sid>', category.loadCategory, name="myadmin_category_load"),#执行编辑
    ...
    path('order/del/<int:oid>', order.delete, name="myadmin_order_delete"), #执行删除
