...
class User(models.Model):
    ...
    status = models.IntegerField(default=1) #状态：1正常2禁用6管理员9删除
...
class Order(models.Model):
    ...
    money = models.FloatField()     #花费金额
    status = models.IntegerField(default=1)        #状态:1正常/2禁用/9删除
    payment_status = models.IntegerField(default=1)        #状态:1正常/2禁用/9删除
    create_at = models.DateTimeField(default=datetime.now)    #创建时间
    update_at = models.DateTimeField(default=datetime.now)    #修改时间
    
    def toDict(self):
        return{'id':self.id,'shop_id':self.shop_id,'user_id':self.user_id,'member_id':self.member_id,'money':self.money,'status':self.status,'payment_status':self.payment_status,'create_at':self.create_at.strftime('%Y-%m-%d %H:%M:%S'),'update_at':self.update_at.strftime('%Y-%m-%d %H:%M:%S')}
